"""Compare gate and audio/vision Transformer runs on matched seeds/views."""
...
def evaluation_path(old_root: Path, new_root: Path, arch: str, exp: str, seed: int, split: str):
    if arch == "gate" and split == "test" and seed != 42:
        return Path("outputs/problem2_cross_attention/legacy_gate_recheck") / f"{exp}_seed{seed}_eval_test.json"
...
def summarize_metric(old_root, new_root, exp, split, key):
    ...
    return {
        ...
        "transformer_mean": float(values["transformer"].mean()),
        "transformer_sd": float(values["transformer"].std(ddof=1)),
        "paired_delta": float((values["transformer"] - values["gate"]).mean()),
        "gate_per_seed": values["gate"].tolist(),
        "transformer_per_seed": values["transformer"].tolist(),
        "delta_per_seed": (values["transformer"] - values["gate"]).tolist(),
    }
...
def condition_deltas(old_root, new_root, exp):
    ...
    for seed in SEEDS:
        ...
        new = read_json(evaluation_path(old_root, new_root, "transformer", exp, seed, "valid"))["condition_summary"]
        if set(old) != set(new):
...
def main():
    ...
    parser.add_argument("--transformer-root", type=Path, default=Path("outputs/problem2_av_transformer"))
    ...
    for exp in EXPERIMENTS:
        check_training_config(args.gate_root, args.transformer_root, exp)
        result["experiments"][exp] = {
            "valid": {key: summarize_metric(args.gate_root, args.transformer_root, exp, "valid", key)
                      for key in METRICS["valid"]},
            "test": {key: summarize_metric(args.gate_root, args.transformer_root, exp, "test", key)
                     for key in METRICS["test"]},
            "paired_validation": paired_predictions(args.gate_root, args.transformer_root, exp, args.bootstrap),
            "condition_f1": condition_deltas(args.gate_root, args.transformer_root, exp),
        }
    args.transformer_root.mkdir(parents=True, exist_ok=True)
    json_path = args.transformer_root / "comparison_with_gate.json"
    md_path = args.transformer_root / "comparison_with_gate.md"
    json_path.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
    lines = [
        "# Gate versus audio/vision temporal Transformer",
        "",
        "Matched E3/E5 training runs, seeds 42/52/62, and fixed validation views.",
        "Positive delta means the temporal Transformer is higher. Lower MAE is better.",
        "",
        "| Experiment | Split and metric | Gate mean +/- SD | Transformer mean +/- SD | Transformer - gate |",
        "|---|---|---:|---:|---:|",
    ]
    table_metrics = (
    ...
    for exp in EXPERIMENTS:
        ...
        for split, key in table_metrics:
            ...
            lines.append(
                ...
                f"{format_mean(row, 'transformer')} | {row['paired_delta']:+.4f} |"
            )
    lines.extend([
        "",
        ...
        "",
        "| Experiment | Subset | Metric | Transformer - gate | 95% cluster interval |",
        "|---|---|---|---:|---:|",
    ])
    for exp in EXPERIMENTS:
    ...
    lines.extend([
        ...
        "## Conclusion",
        "",
        "E3 improves the primary missing36 validation F1 and independent clean test F1.",
        "E5 declines on both; keep the current E5 gate as the default model.",
        "",
        "Source files: each run's config.json, run_info.json, eval_valid.json,",
        "eval_valid_samples.npz and eval_test.json. Historical gate seed-52/62 test",
        "results are under outputs/problem2_cross_attention/legacy_gate_recheck/.",
    ])
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json_path)
    print(md_path)
